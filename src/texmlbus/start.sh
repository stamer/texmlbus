...
# PHP Config
if [ ! -z "$CONTAINER_TIMEZONE" ]; then sed -i "s#\;\?\\s\?date.timezone =.*#date.timezone = '$CONTAINER_TIMEZONE'#" /etc/php7/php.ini && echo "Set PHP date.timezone = $CONTAINER_TIMEZONE..."; fi
